...
import pandas as pd
from src.extraction import extract_text_from_pdf_or_image  # Importing hybrid extraction function from extraction.py
...
def extractor_node(state: AgentState) -> AgentState:
    ...
    # Process each report to extract text (using hybrid method: PDF + OCR)
    for report in reports:
        # Assuming the report contains a 'file_path' key for the uploaded file
        file_path = report.get("file_path", "")
        report_text = extract_text_from_pdf_or_image(file_path)  # Extract text from the report
        extracted_values = extract_lab_values(report_text)  # Extract lab values from the text
        lab_values.extend(extracted_values)  # Add extracted values to the list
